...
Любой класс-наследник обязан реализовать эти методы, иначе при создании экземпляра возникнет ошибка TypeError.
    ...
    Реализация абстрактных методов в дочерних классах Пример для класса HDD (файл concrete_devices.py):

    class HDD(StorageDevice): # Наследование от абстрактного класса def init(self, name, price, capacity_gb, rpm=7200): super().init(name, price, capacity_gb) self._rpm = rpm
...
TypeError: Can't instantiate abstract class HDD with abstract methods get_read_speed, get_write_speed 3. Использование других декораторов В классе StorageDevice также используются:
...
Генерирует автоматически все методы сравнения (>, <=, >= и т.д.) на основе eq и lt.

    Запрет инстанцирования абстрактного класса Попытка создать объект абстрактного класса вызовет ошибку:

python device = StorageDevice("Test", 100, 500) # Ошибка!
TypeError: Can't instantiate abstract class StorageDevice with abstract methods get_read_speed, get_write_speed

Итог Абстрактный класс:
...
StorageDevice (ABC) ├── HDD ├── SSD └── FlashDrive Каждый дочерний класс реализует уникальную логику абстрактных методов.
...
Общая структура наследования StorageDevice (ABC) ├── HDD ├── SSD └── FlashDrive Каждый дочерний класс:
...
Реализует минимум 2 dunder-метода
    ...
    Базовый класс: StorageDevice Файл: base_storage.py

    @total_ordering class StorageDevice(ABC): def init(self, name, price, capacity_gb): self._name = name self._price = price self._capacity_gb = capacity_gb
...
capacity_gb (ёмкость)
    ...
    Дочерний класс: HDD Файл: concrete_devices.py

    class HDD(StorageDevice): def init(self, name, price, capacity_gb, rpm=7200): super().init(name, price, capacity_gb) # Вызов родительского конструктора self._rpm = rpm # Уникальный атрибут
...
add — создание RAID-массива

mul — масштабирование массива

    Дочерний класс: SSD Файл: concrete_devices.py

    class SSD(StorageDevice): def init(self, name, price, capacity_gb, nvme=False): super().init(name, price, capacity_gb) self._nvme = nvme # Уникальный атрибут
...
contains — проверка поддержки технологии

call — вызов объекта как функции

    Дочерний класс: FlashDrive Файл: concrete_devices.py

    class FlashDrive(StorageDevice): def init(self, name, price, capacity_gb, usb_version=3): super().init(name, price, capacity_gb) self._usb_version = usb_version # Уникальный атрибут
...
len — возвращает ёмкость

getitem — доступ к данным через ключи

Ключевые принципы иерархии Единый интерфейс:
...
python devices = [HDD(...), SSD(...), FlashDrive(...)] for device in devices: print(device.get_read_speed()) # Вызовется своя реализация для каждого типа Расширяемость:
...
Итог Иерархия наследования позволяет:
